...
class PaymentHandler:
    async def show_token_packages(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.error(f"Error showing token packages: {e}")
            error_msg = "❌ Error loading packages. Please try again."
            if update.message:
    ...
    async def handle_package_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logger.info(f"💰 Payment selection: {data} from user {user_id}")
        ...
        try:
            ...
            if data.startswith("buy_stars_"):
                ...
                # Stars payment coming soon
                await query.edit_message_text(
                    "💫 **Stars Payment**\n\n"
                    f"⭐ Stars payment is coming soon!\n\n"
                    f"Please use UPI payment for now.",
    ...
    async def initiate_stars_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, package_id: str):
        ...
        try:
            ...
            logger.error(f"Error in stars payment: {e}")
            await query.edit_message_text("❌ Payment initiation failed. Please try again.")
    ...
    async def verify_stars_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, transaction_id: str):
        ...
        try:
            ...
            success_text = (
                ...
                f"**New Balance:** {user.tokens} tokens\n"
            ...
            await query.edit_message_text("❌ Verification failed. Please contact support.")
    ...
    async def verify_upi_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, transaction_id: str):
        ...
        try:
            ...
            all_admins = config.ADMIN_IDS + config.OWNER_IDS
            if config.SUPER_ADMIN_ID and config.SUPER_ADMIN_ID not in all_admins:
                all_admins.append(config.SUPER_ADMIN_ID)
            ...
            logger.error(f"Error in UPI verification: {e}")
            await query.edit_message_text("❌ Verification failed. Please contact support.")
    ...
    async def check_balance(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if transactions:
                ...
                balance_text += "**Recent Transactions:** None\n"
            ...
            error_msg = "❌ Error checking balance. Please try again."
            if update.message:
                await update.message.reply_text(error_msg)
            else:
                await update.callback_query.edit_message_text(error_msg)
    ...
    async def admin_verify_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text("❌ Verification failed. Please check the transaction ID.")